  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-[#01161E] tracking-tight">Employee Reports</h1>
          <p className="text-xs text-[#598392] mt-0.5 font-light font-sans">Workspace demographics, headcount growth, and deactivations summary.</p>
        </div>
      </div>
      ...
      <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-2xl p-6 space-y-6">
        <div className="space-y-3.5 text-xs text-[#598392] font-light">
          <h3 className="font-semibold text-[#01161E] text-sm mb-4">Onboarding Demographics</h3>
          <div className="flex justify-between border-b border-[#E2E8F0] pb-2">
            <span className="text-[#94A3B8]">Total Registered Staff</span>
            <span className="text-[#01161E] font-medium">{data?.totalStaff || 0} Members</span>
          </div>
          <div className="flex justify-between border-b border-[#E2E8F0] pb-2">
            <span className="text-[#94A3B8]">Active Administrators</span>
            <span className="text-[#01161E]">{data?.admins || 0} Admin</span>
          </div>
          <div className="flex justify-between border-b border-[#E2E8F0] pb-2">
            <span className="text-[#94A3B8]">Active Managers</span>
            <span className="text-[#01161E]">{data?.managers || 0} Managers</span>
          </div>
          <div className="flex justify-between border-b border-[#E2E8F0] pb-2">
            <span className="text-[#94A3B8]">Active Employees</span>
            <span className="text-[#01161E]">{data?.employees || 0} Employees</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#94A3B8]">Inactive Accounts</span>
            <span className="text-[#01161E] text-red-400">{data?.inactive || 0} Deactivated</span>
          </div>
        </div>

        {/* Headcount by Department */}
        <div className="pt-4 border-t border-[#E2E8F0] space-y-3.5 text-xs text-[#598392] font-light">
          <h3 className="font-semibold text-[#01161E] text-sm">Headcount by Department</h3>
            ...
            <div className="space-y-2.5">
              {Object.entries(deptCounts).map(([dept, count]) => (
                <div key={dept} className="flex justify-between items-center border-b border-[#E2E8F0]/40 pb-2 last:border-0 last:pb-0">
                  <span className="text-[#598392]">{dept}</span>
                  <span className="text-[#01161E] font-bold bg-[#FFFFFF] px-2 py-0.5 rounded font-mono text-[10px] border border-[#E2E8F0]/20">{count}</span>
                </div>
              ))}
            </div>
          )}
