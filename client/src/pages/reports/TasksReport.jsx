  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-[#0F172A] tracking-tight">Task Completion Reports</h1>
          <p className="text-xs text-[#64748B] mt-0.5 font-light font-sans">Sprint task velocities, completion ratios, and outstanding backlogs.</p>
        </div>
      </div>

      <div className="bg-white border border-[#E2E8F0] rounded-2xl p-6 shadow-sm">
        <div className="space-y-3.5 text-xs text-[#64748B] font-light">
          <h3 className="font-semibold text-[#0F172A] text-sm mb-4">Sprint Completion Rates</h3>
          <div className="flex justify-between border-b border-[#E2E8F0] pb-2">
            <span className="text-[#94A3B8]">Total Workspace Tasks</span>
            <span className="text-[#0F172A] font-medium">{data?.totalTasks || 0} Tasks</span>
          </div>
          <div className="flex justify-between border-b border-[#E2E8F0] pb-2">
            <span className="text-[#94A3B8]">Completed checklist</span>
            <span className="text-emerald-500 font-medium">{data?.completionRate || 0}% ({data?.completedTasks || 0} tasks)</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[#94A3B8]">Outstanding sprint items</span>
            <span className="text-amber-500 font-medium">{data?.outstandingTasks || 0} Tasks</span>
          </div>
        </div>
      </div>
    </div>
