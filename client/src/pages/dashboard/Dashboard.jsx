...
const Dashboard = () => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#FFFFFF] border border-[#EAE8E2] px-3.5 py-2.5 rounded-xl shadow-2xl text-xs space-y-1">
          <p className="font-semibold text-[#0F172A] font-mono">{label || 'Task Summary'}</p>
  ...
  return (
    <div className="space-y-6">
      {/* Welcome header */}
      <div className="pb-5 border-b border-[#EAE8E2] space-y-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Employees', value: totalEmployees, subtitle: 'Managers & Staff', icon: <Users className="h-5 w-5" />, link: '/employees', colorClass: 'bg-blue-500/10 text-blue-500 border-blue-500/20' },
          { label: 'Active Projects', value: totalProjects, subtitle: 'Sprints in progress', icon: <FolderGit2 className="h-5 w-5" />, link: '/projects', colorClass: 'bg-indigo-500/10 text-indigo-500 border-indigo-500/20' },
          { label: 'Open Tasks', value: openTasksCount, subtitle: 'Assigned to team', icon: <CheckSquare className="h-5 w-5" />, link: '/tasks', colorClass: 'bg-amber-500/10 text-amber-500 border-amber-500/20' },
          { label: 'Daily Attendance', value: `${attendanceRate}%`, subtitle: `${todayCheckedInCount} Checked-in Today`, icon: <Clock className="h-5 w-5" />, link: '/attendance', colorClass: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' }
        ].map((card, idx) => (
          <div
            key={idx}
            onClick={() => navigate(card.link)}
            className="bg-white border border-[#EAE8E2]/80 rounded-[20px] p-5 flex flex-col justify-between hover-card shadow-premium cursor-pointer"
          >
            <div className="flex justify-between items-center mb-3">
              <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-wider">{card.label}</span>
              <div className={`h-9 w-9 rounded-xl flex items-center justify-center border shadow-sm ${card.colorClass}`}>
                {card.icon}
              </div>
            </div>
            <div>
              <p className="text-2xl font-bold text-[#0F172A] tracking-tight">{loading ? '...' : card.value}</p>
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8 bg-white border border-[#E2E8F0]/80 rounded-[20px] p-6 space-y-4 shadow-premium hover-card">
              <div className="flex justify-between items-center border-b border-[#E2E8F0]/60 pb-4">
                <div>
                  <h3 className="font-bold text-[#0F172A] text-[15px] font-heading">Active Projects</h3>
                  <p className="text-[10px] text-[#94A3B8] font-medium mt-0.5">Current deliverables and deadlines</p>
                </div>
                <button
                  onClick={() => navigate('/projects')}
                  className="text-[10px] text-[#5A42EC] hover:text-[#0F172A] font-bold flex items-center gap-1 cursor-pointer transition-colors"
                >
                  <span>View All</span>
              ...
              {projectListWithProgress.length === 0 ? (
                ...
                <div className="divide-y divide-[#E2E8F0]/40 text-xs">
                  {projectListWithProgress.map((p) => (
                    <div key={p.id} className="py-3 px-3.5 -mx-3.5 hover:bg-[#F4F5F9]/60 rounded-xl transition-colors duration-150 flex items-center justify-between gap-4">
                      ...
                      <div className="w-48 hidden sm:block">
                        <div className="flex justify-between text-[10px] text-[#94A3B8] mb-1 font-medium font-sans">
                          <span>Progress</span>
                          <span className="text-[#0F172A] font-bold">{p.progress}%</span>
                        </div>
                        <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden border border-slate-200/20">
                          <div className="bg-gradient-to-r from-[#5A42EC] to-[#4831D4] h-full rounded-full transition-all duration-300" style={{ width: `${p.progress}%` }}></div>
                        </div>
                      </div>
            ...
            <div className="lg:col-span-4 bg-white border border-[#E2E8F0]/80 rounded-[20px] p-6 space-y-4 shadow-premium hover-card flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center border-b border-[#E2E8F0]/60 pb-4">
                  <div>
                    <h3 className="font-bold text-[#0F172A] text-[15px] font-heading">Task Distribution</h3>
                    <p className="text-[10px] text-[#94A3B8] font-medium mt-0.5">Sprint task load metrics</p>
                  </div>
                  <TrendingUp className="h-4 w-4 text-[#5A42EC]" />
                </div>
              ...
              {/* Status Legends */}
              <div className="grid grid-cols-2 gap-2.5 text-[11px] text-[#64748B] pt-3 border-t border-slate-100">
                {taskStatusData.length === 0 ? (
                  ...
                  taskStatusData.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-1.5">
                      ...
                      <span className="truncate text-xs font-semibold text-[#64748B]">{item.name} ({item.value})</span>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8 bg-white border border-[#E2E8F0]/80 rounded-[20px] p-6 space-y-4 shadow-premium hover-card">
              <div className="flex justify-between items-center border-b border-[#E2E8F0]/60 pb-4">
                <div>
                  <h3 className="font-bold text-[#0F172A] text-[15px] font-heading">7-Day Attendance Rate</h3>
                  <p className="text-[10px] text-[#94A3B8] font-medium mt-0.5">Staff engagement and presence history</p>
                </div>
                <Calendar className="h-4 w-4 text-[#5A42EC]" />
              </div>
              <div className="h-60 w-full mt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={attendanceTrendData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                    ...
                    <YAxis stroke="#A3A3A3" fontSize={9} domain={[0, 100]} tickLine={false} axisLine={false} tickFormatter={v => `${v}%`} />
                    <Tooltip content={<CustomTooltip />} />
                    <Area type="monotone" dataKey="Attendance Rate (%)" stroke="#111111" strokeWidth={2} fillOpacity={1} fill="url(#colorAttendance)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 bg-white border border-[#E2E8F0]/80 rounded-[20px] p-6 space-y-5 shadow-premium hover-card flex flex-col justify-between">
              <div className="border-b border-[#E2E8F0]/60 pb-4">
                <h3 className="font-bold text-[#0F172A] text-[15px] font-heading">Sprint Performance</h3>
                <p className="text-[10px] text-[#94A3B8] font-medium mt-0.5">Current sprint productivity rate</p>
              </div>

              <div className="text-center p-6 bg-gradient-to-br from-[#F4F5F9] to-[#F1F5F9]/50 border border-slate-100 rounded-2xl my-auto shadow-inner">
                <p className="text-[9px] text-[#94A3B8] uppercase font-bold tracking-wider">Overall Task Velocity</p>
                <p className="text-4xl font-extrabold text-[#0F172A] mt-1 tracking-tight">{taskVelocityRate}%</p>
                <p className="text-[10px] text-green-500 mt-1 font-semibold">Sprint progress target</p>
              </div>

              <div className="space-y-3.5 text-xs text-[#64748B] font-semibold pt-4 border-t border-slate-100">
                <div className="flex justify-between">
                  <span className="text-[#94A3B8]">Completed Tasks</span>
                  <span className="text-[#0F172A]">{completedTasksCount} / {totalTasksCount}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#94A3B8]">Pending Approvals</span>
                  <span className="text-[#0F172A]">{pendingLeavesCount} leaves</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#94A3B8]">Active Projects</span>
                  <span className="text-[#0F172A]">{totalProjects}</span>
                </div>
              </div>
            </div>
          </div>
